...
pub fn record_attendance_input(
    ...
    require!(ticket.purchase_trust_recorded, TicketingError::Unauthorized);
    require!(
        !ticket.attendance_trust_recorded,
    ...
    signal.attendance_eligible_count = signal
        .attendance_eligible_count
        .checked_add(1)
        .ok_or(TicketingError::MathOverflow)?;
    if did_attend {
        signal.attendance_attended_count = signal
    ...
    ticket.attendance_trust_recorded = true;
...
pub struct RecordAttendanceInput<'info> {
    #[account(mut)]
    ...
    #[account(
        ...
        constraint = ticket.owner == wallet.key() @ TicketingError::Unauthorized,
    )]
