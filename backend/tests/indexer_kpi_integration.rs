...
#[tokio::test]
async fn indexer_admin_endpoints_require_protocol_admin() {
    let op_key = SigningKey::from_bytes(&[91u8; 32]);
...
#[tokio::test]
async fn protocol_admin_can_get_indexer_status() {
    let admin_key = SigningKey::from_bytes(&[92u8; 32]);
    let admin_wallet = bs58::encode(admin_key.verifying_key().as_bytes()).into_string();
    let rules = format!(
        r#"[{{"wallet":"{admin_wallet}","role":"protocol_admin","organizer_scopes":["org_123"]}}]"#
    );

    let state = AppState::bootstrap(test_config(rules)).await.unwrap();
    let app = build_router(state);
    let token = auth_token(&app, &admin_key, &admin_wallet).await;
