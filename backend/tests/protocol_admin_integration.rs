...
#[tokio::test]
async fn protocol_admin_endpoint_rejects_non_admin_role() {
    ...
    let req = Request::builder()
        .method("POST")
        .uri("/api/v1/protocol-admin/pause-protocol")
        .header("content-type", "application/json")
...
#[tokio::test]
async fn protocol_admin_simulate_allows_admin_and_returns_chain_error_on_unreachable_rpc() {
    let signing_key = SigningKey::from_bytes(&[22u8; 32]);
    let wallet = bs58::encode(signing_key.verifying_key().as_bytes()).into_string();
    let rules =
        format!(r#"[{{"wallet":"{wallet}","role":"protocol_admin","organizer_scopes":[]}}]"#);
    ...
    let req = Request::builder()
        .method("POST")
        .uri("/api/v1/protocol-admin/set-protocol-config/simulate")
        .header("content-type", "application/json")
        .header("authorization", format!("Bearer {token}"))
        .body(Body::from(
            json!({
