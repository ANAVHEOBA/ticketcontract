...
#[tokio::test]
async fn operator_cannot_transition_ticket_to_refunded() {
    ...
    let req = Request::builder()
        .method("POST")
        .uri("/api/v1/ticket-state/transition")
        .header("content-type", "application/json")
...
#[tokio::test]
async fn policy_validation_endpoint_rejects_bad_config() {
    ...
    let req = Request::builder()
        .method("POST")
        .uri("/api/v1/resale-policy/validate")
        .header("content-type", "application/json")
        .header("authorization", format!("Bearer {token}"))
        .body(Body::from(
            json!({
...
#[tokio::test]
async fn set_policy_forbidden_for_scanner() {
