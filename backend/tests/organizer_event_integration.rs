...
#[tokio::test]
async fn organizer_write_rejects_scanner_role() {
...
#[tokio::test]
async fn event_create_allows_editor_role_and_uses_chain_wrapper() {
...
#[tokio::test]
async fn event_reads_return_db_unavailable_without_db_connection() {
    ...
    let req = Request::builder()
        .method("GET")
        .uri("/api/v1/events?organizer_id=org_123")
        .header("authorization", format!("Bearer {token}"))
        .body(Body::empty())
        .unwrap();
